...
class YouTube:
    ...
    async def get_video_async(youtube_id: str) -> AResult[VideoResponse]:
        ...
        a_result_video: AResult[VideoRow] = await YouTubeAccess.get_video_youtube_id_async(youtube_id=youtube_id)
        if a_result_video.is_ok():
            ...
            a_result_channel: AResult[ChannelRow] = await YouTubeAccess.get_channel_id_async(
                id=video_row.channel_id)
            channel: ChannelRow = a_result_channel.result() if a_result_channel.is_ok() else None
            ...
            if channel:
                ...
                if channel.internal_image_id:
                    a_result_channel_image: AResult[ImageRow] = await MediaAccess.get_image_from_id_async(
        ...
        try:
            async with rockit_db.session_scope_async() as session:
                a_result_channel_row: AResult[ChannelRow] = await YouTubeAccess.get_or_create_channel(
                    raw=raw_channel,
                    provider_id=provider_id,
                    session=session)
                if a_result_channel_row.is_not_ok():
                    return AResult(code=a_result_channel_row.code(), message=a_result_channel_row.message())

                channel_row: ChannelRow = a_result_channel_row.result()

                a_result_video_row: AResult[VideoRow] = await YouTubeAccess.get_or_create_video(
                    raw=raw_video,
                    channel_id=channel_row.id,
                    provider_id=provider_id,
                    session=session)
                if a_result_video_row.is_not_ok():
                    return AResult(code=a_result_video_row.code(), message=a_result_video_row.message())
