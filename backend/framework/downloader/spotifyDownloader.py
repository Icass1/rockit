...
from backend.framework.downloader.messageHandler import MessageHandlderReader, MessageHandler
from backend.framework.downloader.queueElement import QueueElement, SpotifyQueueElement
    ...
    def __init__(self, downloader: "Downloader", download_public_id: str, user_id: int, url: str) -> None:
        ...
        def _func(s: Session):
            download_to_add = DownloadRow(
                ...
                user_id=user_id,
                date_started=datetime.now(tz=timezone.utc),
                download_url=url
            )
