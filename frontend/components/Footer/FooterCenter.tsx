...
import { Shuffle, SkipBack, SkipForward, CirclePlay, CirclePause, Repeat, Repeat1 } from "lucide-react";
import Slider from "@/components/Slider";
import Spinner from "@/components/Spinner";
import { getTime } from "@/lib/utils/getTime";
import { rockIt } from "@/lib/rockit/rockIt";

const ICON_BTN = "cursor-pointer text-gray-400 transition-all md:hover:scale-105 md:hover:text-white";
    ...
    return (
        <div className="hidden w-1/3 flex-col items-center justify-center space-y-1 md:flex">
            ...
            <div className="grid grid-cols-5 items-center justify-items-center gap-2">
                <button
                    aria-label={$randomQueue ? "Disable shuffle" : "Enable shuffle"}
                    onClick={() => rockIt.userManager.toggleRandomQueue()}
                >
                    <Shuffle className={`h-4.5 w-4.5 transition-colors md:hover:scale-105 ${$randomQueue ? ACTIVE : "text-gray-400"}`} />
                </button>

                <button aria-label="Previous song" onClick={() => rockIt.queueManager.skipBack()}>
                    <SkipBack className={`h-5.5 w-5.5 fill-current ${ICON_BTN}`} />
                </button>
                ...
                <button aria-label="Next song" onClick={() => rockIt.queueManager.skipForward()}>
                    <SkipForward className={`h-5.5 w-5.5 fill-current ${ICON_BTN}`} />
                </button>

                <button
                    aria-label={
                        $repeatSong === "one" ? "Repeat one" :
                        $repeatSong === "all" ? "Repeat all" : "No repeat"
                    }
                    onClick={() => rockIt.userManager.cyclerepeatSong()}
                >
                    <RepeatIcon className={`h-4.5 w-4.5 transition-colors md:hover:scale-105 ${isRepeatActive ? ACTIVE : "text-gray-400"}`} />
                </button>
            </div>
            ...
            <div className="flex h-7 w-full items-center space-x-2">
                ...
                <Slider
                    ...
                    step={0.001}
                    onChange={(e) => rockIt.audioManager.setCurrentTime(Number(e.target.value))}
                />
                <span className="min-w-6 text-xs font-semibold tabular-nums">
