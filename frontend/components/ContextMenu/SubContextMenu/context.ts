...
export const SubContextMenuContext = createContext<SubContextMenuProps | null>(null);

export function useSubContextMenu() {
    const ctx = useContext(SubContextMenuContext);
    if (!ctx) throw new Error("useSubContextMenu must be used inside SubContextMenu");
    return ctx;
}
