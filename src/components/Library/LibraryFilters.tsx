...
interface LibraryFiltersProps {
    ...
    setLibraryView: React.Dispatch<React.SetStateAction<"grid" | "byArtist">>;
}
...
export function LibraryFilters({
    ...
    return (
        <div className="flex w-full items-center gap-4 md:w-fit">
            ...
            >
                {filterMode === "default" && (
                    <ClockArrowDown className="h-6 w-6 cursor-pointer text-white" />
                )}
                {filterMode === "asc" && (
                    <ArrowDownAZ className="h-6 w-6 cursor-pointer text-white" />
                )}
                {filterMode === "desc" && (
                    <ArrowUpAZ className="h-6 w-6 cursor-pointer text-white" />
                )}
            </button>
