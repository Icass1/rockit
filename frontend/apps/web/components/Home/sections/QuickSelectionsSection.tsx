...
const SONGS_POOL = 8 * SONGS_PER_COLUMN + SONGS_PER_COLUMN; // 36
...
export default function QuickSelectionsSection({ title, songs }: QuickSelectionsSectionProps) {
    ...
    <section className="text-white md:pl-12">
      ...
      <div className="scrollbar-hide flex snap-x snap-mandatory overflow-x-auto px-8 py-4 md:gap-4 md:px-2 md:[scrollbar-gutter:stable]">
        {Array.from({ length: COLUMNS }).map((_, columnIndex) => (
          ...
          >
            {songs
              .slice(columnIndex * SONGS_PER_COLUMN, columnIndex * SONGS_PER_COLUMN + SONGS_PER_COLUMN)
              .map((song) => (
                <QuickSelectionsSong
