...
from song import song_query, parse_song
from album import album_query, parse_album
from download import download_query
from error import error_query
from user import user_query, parse_user
from playlist import playlist_query, parse_playlist
from image import image_query, parse_image
from session import sesssion_query
...
class Table:
    ...
    def get_all(self, query: str, parameters=None):
        ...
        if self.parser:
            results = [self.parser(dict(zip(columns, result))) for result in results]
        else:
...
class DB:
    ...
    def get_table_difference(self, list_a: List[Dict], list_b: List[Dict]):
        ...
        for col_a in list_a:
            col_b = next((c for c in list_b if c['name'] == col_a['name']), None)
            if col_b is None:
    ...
    def get_table_columns(self, query: str):
        ...
        col_defs = query.split("(", 1)[1].rsplit(")", 1)[0].replace("\n", "").split(",")
    ...
    def get_existing_columns(self, table_name: str):
        ...
        existing_columns = [dict(zip([col[0] for col in self.cur.description], row)) for row in self.cur.fetchall()]
             
        return existing_columns
    
    def get_table_name(self, query: str) -> str | None:
        match = re.search(r"\b(?:FROM|INTO|TABLE(?: IF NOT EXISTS)?|JOIN)\s+([`\"']?)(\w+)\1", query, re.IGNORECASE)
    ...
    def check_table(self, query: str):
        ...
        if modified_columns:
            ...
            if INSECURE_DB_MODE:
                ...
                self.cur.execute(f"ALTER TABLE temp_{table_name} RENAME TO {table_name}")
                self.conn.commit()
