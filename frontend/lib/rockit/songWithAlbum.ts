...
export class SongWithAlbum {
    ...
    constructor({ publicId, name, artists, downloaded, discNumber, duration, album, internalImageUrl, audioUrl }: {
        publicId: string;
        name: string;
        artists: Artist[];
        discNumber: number;
    ...
    static fromResponse(
        ...
        const existing = SongWithAlbum.#instance.find((s) => s.publicId === response.publicId);
