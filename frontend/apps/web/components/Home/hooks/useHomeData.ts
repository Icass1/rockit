...
function transformStats(
  ...
  const songsByTimePlayed = dataResponse.songsByTimePlayed.map((song) => song);
  const randomSongsLastMonth = dataResponse.randomSongsLastMonth.map((song) => song);
...
export function useHomeData(
  initialStats?: Awaited<ReturnType<typeof HomeStatsResponseSchema.parse>> | null
): HomeData | null {
