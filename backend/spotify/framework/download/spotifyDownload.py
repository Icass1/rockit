...
class SpotifyDownload(BaseDownload):
    ...
    async def download_method_async(self) -> AResultCode:
        ...
        if a_result_spotdl_song.is_not_ok():
            ...
            return AResultCode(
                code=a_result_spotdl_song.code(),
                message=a_result_spotdl_song.message()
            )
        ...
        spotify_downloader.progress_handler.downloads_ids_dict[song_name] = spotdl_song.song_id
        ...
        try:
            ...
            out_song, song_temp_path = await asyncio.to_thread(
                spotify_downloader.spotdl_downloader.search_and_download,
                spotdl_song
            )
            out_song: SpotdlSong
            ...
            return AResultCode(
                code=AResultCode.GENERAL_ERROR,
                message=f"Error downloading song: {e}"
            )

        finally:
            # Clean up dictionaries after download finishes.
            spotify_downloader.progress_handler.downloads_ids_dict.pop(
                song_name, None)
