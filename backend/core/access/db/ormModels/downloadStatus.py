...
from backend.core.access.db.ormModels.declarativeMixin import (
    TableAutoincrementId, TableDateAdded, TableDateUpdated)

if TYPE_CHECKING:
    from backend.core.access.db.ormModels.download import DownloadRow


class DownloadStatusRow(CoreBase, TableAutoincrementId, TableDateUpdated, TableDateAdded):
    __tablename__ = 'download_status'
    __table_args__ = {'schema': 'core', 'extend_existing': True},

    download_id: Mapped[int] = mapped_column(
        Integer,
        ForeignKey('core.download.id'), nullable=False)
    ...
    download: Mapped["DownloadRow"] = relationship(
        "DownloadRow",
        back_populates="download_status_list"
    )
