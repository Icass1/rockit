// This file is generated using: python3 -m backend import_vocabulary
// Do not modify this file manually.

export interface Vocabulary {
    ADD_LIST_RANDOMLY: string;
    ADD_LIST_TO_BOTTOM: string;
    ADD_LIST_TO_QUEUE: string;
    ADD_MEDIA_TO_PLAYLIST: string;
    ADD_SONG_TO_PLAYLIST: string;
    ADD_SONG_TO_QUEUE: string;
    ADD_TO_LIBRARY: string;
    ADD_TO_LIKED: string;
    ADD_TO_QUEUE: string;
    ADDED_TO_PLAYLIST: string;
    ADMIN_ADD_BUILD: string;
    ADMIN_ANONYMOUS: string;
    ADMIN_APK_FILE: string;
    ADMIN_AVG_RESPONSE_TIME: string;
    ADMIN_AVG_TIME: string;
    ADMIN_BUILDS_SUBTITLE: string;
    ADMIN_BUILDS_TITLE: string;
    ADMIN_CANCEL: string;
    ADMIN_COUNT: string;
    ADMIN_DESCRIPTION: string;
    ADMIN_DESCRIPTION_PLACEHOLDER: string;
    ADMIN_ENTER_VERSION: string;
    ADMIN_FAILED_TO_LOAD_STATS: string;
    ADMIN_HOURLY_ACTIVITY: string;
    ADMIN_HTTP_METHODS: string;
    ADMIN_IP: string;
    ADMIN_LATEST: string;
    ADMIN_MAX: string;
    ADMIN_METHOD: string;
    ADMIN_MIN: string;
    ADMIN_NO_BUILDS: string;
    ADMIN_NO_DATA_DESC: string;
    ADMIN_NO_DATA_YET: string;
    ADMIN_NO_STATS: string;
    ADMIN_ONLY_APK: string;
    ADMIN_OPTIONAL: string;
    ADMIN_P50_P90: string;
    ADMIN_P95_LATENCY: string;
    ADMIN_P99: string;
    ADMIN_READ_FAILED: string;
    ADMIN_REQ_PER_DAY: string;
    ADMIN_REQUEST_DESCRIPTION: string;
    ADMIN_REQUEST_LOG_STATS: string;
    ADMIN_REQUESTS: string;
    ADMIN_REQUESTS_OVER_TIME: string;
    ADMIN_REQUIRED: string;
    ADMIN_RESPONSE_CODES: string;
    ADMIN_ROUTE: string;
    ADMIN_SELECT_APK: string;
    ADMIN_SELECTED: string;
    ADMIN_SETTINGS_COMING_SOON: string;
    ADMIN_SETTINGS_SUBTITLE: string;
    ADMIN_SETTINGS_TITLE: string;
    ADMIN_TAB_BUILDS: string;
    ADMIN_TAB_SETTINGS: string;
    ADMIN_TAB_USERS: string;
    ADMIN_TOP_IPS: string;
    ADMIN_TOP_ROUTES: string;
    ADMIN_TOTAL_REQUESTS: string;
    ADMIN_UPLOAD_BUILD: string;
    ADMIN_UPLOAD_FAILED: string;
    ADMIN_UPLOAD_FIRST: string;
    ADMIN_UPLOAD_NEW_BUILD: string;
    ADMIN_UPLOAD_SUCCESS: string;
    ADMIN_UPLOADING: string;
    ADMIN_USER: string;
    ADMIN_USER_ACTIVITY: string;
    ADMIN_USERS_COMING_SOON: string;
    ADMIN_USERS_SUBTITLE: string;
    ADMIN_USERS_TITLE: string;
    ADMIN_VERSION: string;
    ALBUM: string;
    ALBUMS: string;
    ALBUMS_SINGLES: string;
    ALL: string;
    ALL_FIELDS_REQUIRED: string;
    ALREADY_LOGGED_IN: string;
    APRIL: string;
    ARTIST: string;
    ARTISTS: string;
    AUGUST: string;
    AVERAGE_MINUTES_PER_SONG: string;
    BY: string;
    CANCEL: string;
    CHANGE_PASSWORD: string;
    CLEAR_DOWNLOADS: string;
    CLICK_TO_DOWNLOAD: string;
    COMMUNITY_TOP: string;
    COMPLETED: string;
    COPY_LIST_URL: string;
    COPY_SONG_URL: string;
    CREATE: string;
    CREATE_ACCOUNT: string;
    CREATING: string;
    DATE_ADDED: string;
    DECEMBER: string;
    DEVICE_DOESNT_SUPPORT_SERVICE_WORKER: string;
    DISC: string;
    DISPLAY_NAME: string;
    DOWNLOAD: string;
    DOWNLOAD_APP: string;
    DOWNLOAD_INPUT_PLACEHOLDER: string;
    DOWNLOAD_LIST_TO_DEVICE: string;
    DOWNLOAD_LIST_TO_SERVER: string;
    DOWNLOAD_MEDIA_TO_DEVICE: string;
    DOWNLOAD_MP3: string;
    DOWNLOAD_SONG_TO_DEVICE: string;
    DOWNLOAD_STARTED: string;
    DOWNLOAD_ZIP: string;
    DOWNLOADING: string;
    DOWNLOADS: string;
    DURATION: string;
    ENERGY: string;
    ENTER_NEW_PLAYLIST_NAME: string;
    ERROR: string;
    ERROR_CHANGING_LANGUAGE: string;
    ERROR_CREATING: string;
    ERROR_CREATING_PLAYLIST: string;
    ERROR_ENTER_NAME: string;
    ERROR_GETTING_DOWNLOADS: string;
    ERROR_GETTING_MEDIA: string;
    ERROR_GETTING_QUEUE: string;
    ERROR_GETTING_USER: string;
    ERROR_LIKING_MEDIA: string;
    ERROR_LOADING_MEDIA_FILE: string;
    ERROR_LOGIN: string;
    ERROR_REGISTER: string;
    ERROR_SEARCHING: string;
    ERROR_STARTING_DOWNLOAD: string;
    ERROR_UNLIKING_MEDIA: string;
    FAILED: string;
    FAILED_TO_ADD_MEDIA_TO_LIBRARY: string;
    FAILED_TO_ADD_MEDIA_TO_PLAYLIST: string;
    FAILED_TO_CHANGE_LANGUAGE: string;
    FAILED_TO_FETCH_PLAYLISTS: string;
    FEATURED_ALBUMS: string;
    FEATURED_LISTS: string;
    FEBRUARY: string;
    FETCHING: string;
    FOCUS: string;
    FRIENDS: string;
    FRIENDS_LIST: string;
    FRIENDS_POINTS_DESCR: string;
    FRIENDS_STATS: string;
    GENERAL: string;
    GENERAL_STATS: string;
    GET_INFO: string;
    GETTING_INFO: string;
    GO_HOME: string;
    GO_TO_ALBUM: string;
    GO_TO_ARTIST: string;
    HIDDEN_GEMS: string;
    HOME: string;
    IN_PROGRESS: string;
    JANUARY: string;
    JULY: string;
    JUNE: string;
    LANGUAGE: string;
    LANGUAGE_CHANGE_FAILED: string;
    LANGUAGE_CHANGED: string;
    LATEST_DOWNLOADS: string;
    LEVEL_ABBR: string;
    LIBRARY: string;
    LIKE: string;
    LIKED_SONGS: string;
    LOG_IN: string;
    LOG_IN_TO: string;
    LOG_OUT: string;
    LOGGING_IN: string;
    LYRICS: string;
    LYRICS_BY: string;
    MARCH: string;
    MAY: string;
    MEDIA_ADDED_TO_LIBRARY: string;
    MEDIA_LIKED: string;
    MEDIA_PROGRESS: string;
    MINUTES: string;
    MINUTES_LISTEND: string;
    MINUTES_LISTENED_PER_DAY: string;
    MOOD_SONGS: string;
    MORE_OPTIONS: string;
    MORE_SONGS_FROM: string;
    MOST_LISTENED: string;
    MOST_LISTENED_ALBUMS: string;
    MOST_LISTENED_ARTISTS: string;
    MOST_LISTENED_SONGS: string;
    NAME: string;
    NEW_PASSWORD: string;
    NEW_PLAYLIST: string;
    NEW_PLAYLIST_NAME: string;
    NEW_VERSION_AVAILABLE: string;
    NEXT_MEDIA: string;
    NO_ALBUMS: string;
    NO_DATA: string;
    NO_DOWNLOADS: string;
    NO_LYRICS: string;
    NO_MEDIA_PLAYING: string;
    NO_PLAYLISTS: string;
    NO_REPEAT: string;
    NO_RESULTS: string;
    NO_SERVICE_WORKER: string;
    NO_SONGS: string;
    NO_STATE: string;
    NO_STATIONS: string;
    NO_VIDEOS: string;
    NOVEMBER: string;
    OCTOBER: string;
    OPEN_LIST: string;
    OPEN_SONG: string;
    OR_CREATE_ACCOUNT: string;
    OR_LOG_IN: string;
    PARTY: string;
    PASSWORD: string;
    PASSWORDS_DONT_MATCH: string;
    PAUSE_SONG: string;
    PENDING: string;
    PENDING_REQUESTS: string;
    PIN: string;
    PINNED_LISTS: string;
    PLACEHOLDER_ALBUM: string;
    PLACEHOLDER_ARTIST: string;
    PLACEHOLDER_PLAYLIST_NAME: string;
    PLACEHOLDER_SONG: string;
    PLAY: string;
    PLAY_LIST: string;
    PLAY_MEDIA: string;
    PLAY_NEXT: string;
    PLAY_SONG: string;
    PLAYLISTS: string;
    PREVIOUS_MEDIA: string;
    QUEUE: string;
    QUICK_SELECTIONS: string;
    RADIO: string;
    RADIO_EMPTY1: string;
    RADIO_EMPTY2: string;
    RADIO_SEARCH: string;
    RADIO_STATIONS: string;
    RECENT_MIX: string;
    RECENT_PLAYED: string;
    RECENTLY_PLAYED: string;
    REGISTER: string;
    REGISTERED: string;
    REGISTERING: string;
    RELATED_ARTISTS: string;
    RELAXED: string;
    REMEMBER_ME: string;
    REMOVE_FROM_LIBRARY: string;
    REMOVE_FROM_LIKED: string;
    REMOVE_FROM_PLAYLIST: string;
    REMOVE_FROM_QUEUE: string;
    REPEAT_ALL: string;
    REPEAT_ONE: string;
    REPEAT_PASSWORD: string;
    RETRY: string;
    RETURN_BACK_HOME: string;
    SAVING: string;
    SEARCH: string;
    SEARCH_BAR: string;
    SEARCH_EMPTY1: string;
    SEARCH_EMPTY2: string;
    SEARCH_LIBRARY: string;
    SEARCH_USERS: string;
    SEND_SONG: string;
    SEPTEMBER: string;
    SETTINGS: string;
    SHARE_SONG: string;
    SHARED_2_YOU: string;
    SHARED_FROM: string;
    SHOWING_DATA: string;
    SIGN_UP: string;
    SONG: string;
    SONGS: string;
    SONGS_FOR_YOU: string;
    SONGS_FROM: string;
    SONGS_LISTENED: string;
    STARTING: string;
    STATISTICS: string;
    STATS: string;
    STOP: string;
    TO: string;
    TODO: string;
    TOP_ALBUMS: string;
    TOP_SONGS: string;
    TOP_VIDEOS: string;
    UNPIN: string;
    UNREGISTER: string;
    UNREGISTERED: string;
    UNREGISTERING: string;
    UPDATE: string;
    UPDATING: string;
    UPLOAD: string;
    USER: string;
    USER_NAME_AND_PASSWORD_REQUIRED: string;
    USER_SETTINGS: string;
    USER_STATS: string;
    USERNAME: string;
    USERS_FRIENDS: string;
    USERS_LIST: string;
    VIDEOS: string;
    WAITING_FOR_QUEUE_SETUP: string;
    YOU_MIGHT_LIKE: string;
    YOUR: string;
    YOUR_ALBUMS_PLAYLISTS: string;
    YOUR_MIX: string;
    YT_VIDEOS: string;
    PLAYLIST: string;
    VIDEO: string;
    RADIO_STATION: string;
    UPLOAD_ACTION: string;
    UPLOAD_ADD_MORE: string;
    UPLOAD_ALBUM: string;
    UPLOAD_ALBUM_TITLE: string;
    UPLOAD_ARTIST_FIELD: string;
    UPLOAD_BROWSE: string;
    UPLOAD_COVER_HINT: string;
    UPLOAD_DROP_ZONE: string;
    UPLOAD_ERROR: string;
    UPLOAD_FILES_READY: string;
    UPLOAD_HINT: string;
    UPLOAD_IN_PROGRESS: string;
    UPLOAD_NO_FILES: string;
    UPLOAD_SONGS: string;
    UPLOAD_TITLE: string;
    UPLOAD_TITLE_FIELD: string;
    UPLOAD_TRACK_FIELD: string;
    UPLOAD_VIDEO: string;
    UPLOAD_VIDEO_TITLE: string;
    DELETE_ERROR: string;
    DELETE: string;
    DELETE_SUCCESS: string;
    REDOWNLOAD: string;
    JUST_NOW: string;
    NOTHING_PLAYING_YET: string;
    FRIENDS_ACTIVITY_WILL_APPEAR: string;
    NOW_PLAYING: string;
    RECENT: string;
    FRIENDS_LISTENING_NOW: string;
    SEE_WHAT_WORLD_LISTENING: string;
    FRIEND_REQUEST: string;
    FRIEND_REQUESTS: string;
    LIVE: string;
    LISTENING_WITH: string;
    LEAVE_SESSION: string;
    ACTIVE_SESSIONS: string;
    ACTIVE: string;
    ENDED: string;
    JOIN: string;
    NO_ACTIVE_SESSIONS: string;
    INVITE_FRIEND_LISTEN: string;
    INVITE_A_FRIEND: string;
    RECEIVED: string;
    SENT: string;
    NO_SHARED_MEDIA_YET: string;
    NOTHING_SHARED_YET: string;
    FROM_USER: string;
    TO_USER: string;
    LEVEL: string;
    XP: string;
    LEADERBOARD: string;
    RESULTS: string;
    NO_USERS_FOUND: string;
    ALREADY_FRIENDS_LABEL: string;
    REQUEST_SENT_LABEL: string;
    ADD_FRIEND: string;
    MY_FRIENDS_TITLE: string;
    NO_FRIENDS_YET: string;
    SEARCH_ABOVE_FIND: string;
    ONLINE_LABEL: string;
    OFFLINE_LABEL: string;
    REMOVE_FRIEND: string;
    SHARE: string;
    TIME_MINUTES: string;
    TIME_HOURS: string;
    TIME_DAYS: string;
    LISTENING_LIVE: string;
    FRIENDS_ACTIVITY: string;
    FRIENDS_FRIENDS: string;
    FRIENDS_SHARE: string;
    FRIENDS_PARTY: string;
    FRIENDS_RANKS: string;
    FRIENDS_SEARCH_PLACEHOLDER: string;
    FRIENDS_NO_FRIENDS: string;
    FRIENDS_PENDING: string;
    FRIENDS_ADD: string;
    FRIENDS_ALREADY_FRIENDS: string;
    FRIENDS_FRIEND_REQUESTS: string;
    FRIENDS_NO_ACTIVITY: string;
    FRIENDS_ONLINE: string;
    FRIENDS_OFFLINE: string;
    FRIENDS_REMOVE: string;
    FRIENDS_INVITE: string;
    FRIENDS_LEAVE: string;
    FRIENDS_NO_SESSIONS: string;
    FRIENDS_ACTIVE: string;
    FRIENDS_JOIN: string;
    FRIENDS_LEADERBOARD: string;
    FRIENDS_SHARE_RECEIVED: string;
    FRIENDS_SHARE_SENT: string;
    FRIENDS_NO_SHARED: string;
    FRIENDS_LIVE: string;
    FRIENDS_MY_FRIENDS: string;
}
