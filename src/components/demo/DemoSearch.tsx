...
function RenderTrackSearchResults({
    tracksInfo,
    handleDownload,
}: {
    tracksInfo: SpotifyTrack[];
    ...
    return (
        <div className="grid grid-cols-2 gap-2 mt-2 mb-5 h-fit">
            {tracksInfo.map((element, index) => (
                ...
                >
                    ...
                    {element.inDatabase && (
                        <CheckIcon className="w-8 text-green-400" />
                    )}
                </div>
            ))}
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 mt-2 mb-5 h-fit">
            {playlistInfo.map((element, index) => (
                ...
                >
                    <img
                        className="aspect-square w-auto h-full "
                        src={element.images[0]?.url || "/song-placeholder.png"}
                    ></img>
                    <div className="flex flex-col text-white min-w-0 max-w-full w-full">
                        <label className="text-base font-semibold truncate">
                            {element.name}
        ...
        <div className="grid grid-cols-2 gap-2 mt-2 mb-5 h-fit">
            {artistInfo.map((element, index) => (
                ...
                >
                    <img
                        src={element.images[0]?.url || "/song-placeholder.png"}
                        className="h-12 rounded-full w-auto aspect-square"
                    />
...
export default function DemoSearch({}) {
            ...
            if (response.ok) {
                response.json().then((json) => {
                    for (let downloadId of json) {
                        console.log(
                            "EventSource",
                            `http://localhost:8000/download-status/${downloadId}`
                        );
    ...
    const handleDownload = (url: string) => {
        fetch(`http://localhost:8000/start-download?url=${url}`)
            .then((data) => data.json())
            .then((json) => {
                const eventSource = new EventSource(
                    `http://localhost:8000/download-status/${json.download_id}`
                );
                eventSource.onmessage = (event) => {
                    onMessage(event, eventSource);
                };
                ...
                {searchResults ? (
                    <div className="w-full min-w-0 max-w-full">
                        ...
                        <RenderTrackSearchResults
                            tracksInfo={searchResults.songs}
                        ...
                        <RenderAlbumSearchResults
                            albumsInfo={searchResults.albums}
                            handleDownload={handleDownload}
                        />
                        <label className="font-bold text-xl text-white ">
                            Playlists
                        ...
                        <RenderPlaylistSearchResults
                            playlistInfo={searchResults.playlists}
                            handleDownload={handleDownload}
                        />
                        <RenderArtistSearchResults
                            artistInfo={searchResults.artists}
                        />
                    </div>
                ) : (
