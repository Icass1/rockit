...
class SpotDL:
    ...
    async def get_spotdl_song_from_song_row(track_row: TrackRow) -> AResult[SpotdlSong]:
        ...
        if a_result_track_artists.is_not_ok():
            ...
            return AResult(code=a_result_track_artists.code(), message=a_result_track_artists.message())

        track_artists: List[ArtistRow] = a_result_track_artists.result()

        a_result_album: AResult[AlbumRow] = await SpotifyAccess.get_album_id_async(id=track_row.album_id)
        if a_result_album.is_not_ok():
            logger.error(
                f"Error getting album from id. {a_result_album.info()}")
            return AResult(code=a_result_album.code(), message=a_result_album.message())

        album: AlbumRow = a_result_album.result()

        a_result_album_artists: AResult[List[ArtistRow]] = await SpotifyAccess.get_artists_from_album_id_async(album_id=track_row.album_id)
        if a_result_album_artists.is_not_ok():
            logger.error(
                f"Error getting artists from album row {a_result_album_artists.info()}")
            return AResult(code=a_result_album_artists.code(), message=a_result_album_artists.message())
        ...
        a_result_core_album: AResult[CoreAlbumRow] = await MediaAccess.get_album_from_id_async(id=track_row.album_id)
