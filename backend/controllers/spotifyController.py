...
async def get_spotify_song(song_public_id: str) -> RockItSongWithAlbumResponse:
...
def get_spotify_playlist(playlist_public_id: str, update: str | None = None) -> RockItPlaylistResponse:
    ...
    a_result_playlist: AResult[PlaylistRow] = downloader.spotify.get_playlist(
        public_id=playlist_public_id, update=False)

    return RockItPlaylistResponse.from_row(playlist=a_result_playlist.result())
