...
def add_albums():
    ...
    for album in tqdm(albums, desc="Adding albums"):
        ...
        internal_image_public_id = album[3]
        name = album[4]
        ...
        if internal_image_public_id == "":
            logger.error(
                f"Skipping album {public_id} with empty internal_image_public_id")
            continue
        ...
        if not internal_image_id:
            logger.error(
                f"Skipping album because it has a internal_image_public_id but an InternalImage was not found")
            continue
        ...
        if not album_id:
            ...
            lists_in_db.append(list_to_add)
...
def download_and_add_to_db_albums_data(ids: List[str]):
    ...
    for album in tqdm(get_spotify_data(ids, "album"), desc="Downloading albums data"):
        ...
        if not isinstance(image_url, str):
            logger.error(
                f"image_url of album {album} is not str, {image_url=}")
            continue
...
def download_and_add_to_db_songs_data(ids: List[str]):
    ...
    for song in tqdm(get_spotify_data(ids, "track"), desc="Downloading songs data"):
        ...
        if not isinstance(album_id, int):
            logger.error(
                f"Skipping song {public_id=} album_id is not int, {album_id=}")
            continue
...
def add_songs():
    ...
    for song in tqdm(songs, desc="Adding songs"):
        ...
        if not internal_image_id:
            logger.error(f"internal_image_id of song {id} is None")
            continue
        ...
        if not album_id:
            logger.error(f"album_id of song {id} is None")
            continue
...
def add_artists():
    ...
    for artist in tqdm(get_spotify_data(unique_artists, "artist"), desc="Downloading artists data"):
        ...
        internal_image_id: str | None = None

        logger.error("Not implemented error")

        if not internal_image_id:
            logger.error(f"internal_image_id of artist {artist['id']} is None")
            continue
...
def add_song_artists():
    ...
    for song_id, artist_public_id in tqdm(song_artists_list, desc="Adding song artists"):
        ...
        if song_id == None:
            logger.error(f"song_id is None, {artist_public_id}")

        if artist_public_id == None:
            logger.error(f"artist_public_id is None, {song_id}")
